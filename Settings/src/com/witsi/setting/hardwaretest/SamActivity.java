...
public class SamActivity extends Activity implements OnClickListener, Callback{
	
	private final String TAG = "SamActivity";
	private Context context = SamActivity.this;
	
	private Button button_sam_return;
	private Button button_sam_ok;
	private Button button_sam_false;
	private Button button_sam_test;
	private TextView tv_tip;
	private ImageView sam_image;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (isburning == true) {
			...
			main = LayoutInflater.from(context).inflate(
					R.layout.hardware_sam_activity, null);
			main.setSystemUiVisibility(View.SYSTEM_UI_FLAG_HIDE_NAVIGATION);
			setContentView(main);
		} else {
			setContentView(R.layout.hardware_sam_activity);
		}
	...
	private void initViews() {
		...
		button_sam_return = (Button) this.findViewById(R.id.back);
		button_sam_ok = (Button) this.findViewById(R.id.pass);
		button_sam_false = (Button) this.findViewById(R.id.fail);
		button_sam_test = (Button) this.findViewById(R.id.test);
		button_sam_return.setOnClickListener(SamActivity.this);
		button_sam_ok.setOnClickListener(SamActivity.this);
		button_sam_false.setOnClickListener(SamActivity.this);
		button_sam_test.setOnClickListener(SamActivity.this);
		tv_tip.setText("");
	}
	...
	private void initDatas(){
		...
		//屏幕休眠
		sam_image = (ImageView) this.findViewById(R.id.sam_image);
		if (config.getBoolean("light", true) == false) {
			sam_image.setBackgroundResource(R.drawable.bg_black);
			screen_sleep = true;
		} else
			screen_sleep = false;
		...
		showTimer.schedule(new TimerTask() {
			...
			public void run() {
				...
				if(flag_sam != 0){
					if(isburning == true){
						exitThread();
						editor.putString("sam", "ng");
						editor.putInt("error_sam", config.getInt("error_sam", 0) + 1);
						editor.putInt("error", 1);
						editor.commit();
						ActivityManagers.toNextBurnTest(context, config, ActivityManagers.BURN_RF);
						finish();
...
//			isThreadRunning = false;
				...
				handler.postDelayed(new Runnable() {
					...
					public void run() {
						...
						if(flag_sam != 0){
							...
							ActivityManagers.startNextActivity(SamActivity.this);
						}
					}
				}, 500);
			} 
	...
	public void onClick(View v) {
		if (screen_sleep == true) {
			sam_image.setBackgroundResource(R.drawable.bg_transport);
			screen_sleep = false;
		} else {
			ItemId = v.getId();
			isburning = false;
			...
			case R.id.back: {
				...
				if (config.getBoolean("singletest", false) == true) {
					ActivityManagers.trunToSingleTestActivity(SamActivity.this);
				} else if (config.getBoolean("alltest", false) == true) {
					ActivityManagers.clearActivity();
					ActivityManagers.trunToEntryActivity(SamActivity.this);
				}else {
					ActivityManagers.trunToBurnStartActivity(SamActivity.this);
				} 
				break;
			}
			case R.id.pass: {
