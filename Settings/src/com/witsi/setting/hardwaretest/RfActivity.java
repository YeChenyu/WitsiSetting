...
public class RfActivity extends Activity implements OnClickListener, Callback{
	
	private final String TAG = "RfActivity";
	private Context context = RfActivity.this;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (isburning == true) {
			...
			main = LayoutInflater.from(context).inflate(
					R.layout.hardware_rf_activity, null);
			main.setSystemUiVisibility(View.SYSTEM_UI_FLAG_HIDE_NAVIGATION);
			setContentView(main);
		} else {
			setContentView(R.layout.hardware_rf_activity);
		}
	...
	private void initViews() {
		...
		button_icc_return.setOnClickListener(RfActivity.this);
		button_icc_ok.setOnClickListener(RfActivity.this);
		button_icc_false.setOnClickListener(RfActivity.this);
		button_icc_test.setOnClickListener(RfActivity.this);
	}
...
//		setCardCheckTimeOut();
			...
			public void run() {
				...
				if (ret < 0) {
					...
					if(flag_icc != 0){
						if(isburning == true){
							exitThread();
							editor.putString("rf", "ng");
							editor.putInt("error_rf", config.getInt("error_rf", 0) + 1);
							editor.putInt("error", 1);
							editor.commit();
							ActivityManagers.toNextBurnTest(context, config, ActivityManagers.BURN_LED);
							finish();
	...
	private synchronized void cardCheck(int slot){
		...
		if (ret < 0) {
			...
			FyLog.i(TAG, "icc power down success.");
			handler.sendEmptyMessage(POWER_DOWN);
		}
	}
	...
	private static final int POWER_DOWN = 7;
	@Override
				...
				handler.postDelayed(new Runnable() {
					...
					public void run() {
						...
						if(flag_icc != 0){
							...
							ActivityManagers.startNextActivity(RfActivity.this);
						}
					}
				}, 500);
			} 
		...
		if (screen_sleep == true) {
			...
			case R.id.back: {
				...
				if (config.getBoolean("singletest", false) == true) {
					ActivityManagers.trunToSingleTestActivity(RfActivity.this);
				} else if (config.getBoolean("alltest", false) == true) {
					ActivityManagers.clearActivity();
					ActivityManagers.trunToEntryActivity(RfActivity.this);
				}else {
					ActivityManagers.trunToBurnStartActivity(RfActivity.this);
				} 
				break;
			}
			case R.id.pass: {
