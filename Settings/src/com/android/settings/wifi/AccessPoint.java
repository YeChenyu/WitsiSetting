...
class AccessPoint extends Preference {
    ...
    PskType pskType = PskType.UNKNOWN;	//接入点密码类型

    private WifiConfiguration mConfig;	//WIFI配置信息
    /* package */ScanResult mScanResult;

    private int mRssi;
    private WifiInfo mInfo;	//当前连接WIFI信息
    ...
    /**
     * 获取安全类型详细描述
     * get the string of Security type
     * @param concise 是否是缩写
     * @return
     */
    ...
    public int compareTo(Preference preference) {
        ...
        // Sort by signal strength. 对比两个网络的信号强度
        int difference = WifiManager.compareSignalLevel(other.mRssi, mRssi);
        if (difference != 0) {
            return difference;
        }
        // Sort by ssid.
        return ssid.compareToIgnoreCase(other.ssid);//不考虑大小写
    }
